...
namespace WA_Progetto
{
    internal class LibraryQuery
    {
        ...
        public int GetFirstFreeID(string tableName, string q, SqlConnection cnn)
        {
            int n = 1;
            string query = $"SELECT MAX({q}) FROM {tableName}";
            try
            {
                using (SqlCommand cmd = new SqlCommand(query, cnn))
                {
        ...
        public Tuple<string, string> GetReferencedTableAndColumn(string tableName, string fkColumn, SqlConnection cnn)
        {
            string refTable = null;
            string refColumn = null;
            string query = @"SELECT PK.TABLE_NAME AS ReferencedTable, PK.COLUMN_NAME AS ReferencedColumn
                        FROM INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS RC
                ...
                using (SqlCommand cmd = new SqlCommand(query, cnn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            refColumn = reader["ReferencedColumn"].ToString();
                        }
                    }
                }
            }
            ...
            return Tuple.Create(refTable, refColumn);
        }
    }
}
